...
async function benchmarkFrameworkOverhead() {
  ...
  console.log(
    "Comparing fp-clean operations with plain JavaScript equivalents\n",
  );
  ...
  const categories = {
    "Value Wrapping": [
      "Plain JS - return value",
      "F.ok - wrap value in Operation",
      "F.err - wrap error in Operation",
    ],
    "Function Application": [
      "Plain JS - function application",
      "F.map - map over Operation",
      "F.flatMap - flatMap over Operation",
    ],
    Composition: [
      "Plain JS - function composition",
      "F.gen - generator composition",
      "Manual composition with flatMap",
    ],
    Execution: ["Execute plain JS", "Execute Operation via get"],
    Environment: [
      "Plain JS - use environment",
      "F.asks - read from environment",
    ],
  };
  ...
  if (plainJsValue && fOkValue) {
    ...
    console.log(
      `F.ok is ${overhead.toFixed(0)}% slower than plain JS value return`,
    );
  }
  ...
  if (plainJsFunc && fMapValue) {
    ...
    console.log(
      `F.map is ${overhead.toFixed(0)}% slower than plain JS function application`,
    );
  }
  ...
  if (plainJsExec && fGetValue) {
    ...
    console.log(
      `Operation execution (get) is ${overhead.toFixed(1)}x slower than plain JS execution`,
    );
  }
}
