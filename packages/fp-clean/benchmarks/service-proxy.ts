...
async function benchmarkServiceProxy() {
  ...
  bench.add('7. Execute operation via get (manual flatMap)', async () => {
    const op = F.flatMap((service: MathService) => service.add(5, 3))(F.askFor(MathTag));
  ...
  if (vanilla) {
    console.log(`Baseline (vanilla): ${(vanilla / 1000).toFixed(3)} µs per call`);
    ...
    for (const [name, mean] of results) {
      ...
      console.log(`\n${name}:`);
      console.log(`  ${(mean / 1000).toFixed(3)} µs per call (${ratio.toFixed(2)}x slower)`);
  ...
  if (manual && proxy) {
    ...
    console.log(`Overhead: ${(proxy - manual).toFixed(0)} ns (${(proxy / manual).toFixed(2)}x)`);
  }
  
  // Compare execution
  const execManual = results.get('7. Execute operation via get (manual flatMap)');
  ...
  if (execManual && execProxy) {
    ...
    console.log(`Manual flatMap execution: ${(execManual / 1000).toFixed(3)} µs`);
    console.log(`Execution overhead: ${(execProxy - execManual).toFixed(0)} ns (${(execProxy / execManual).toFixed(2)}x)`);
  }
}
