...
async function benchmarkMicroOperations() {
  ...
  const objectWithIterator = {
    [Symbol.iterator]: function* () { yield 42; }
  };
  const functionWithIterator = () => 42;
  (functionWithIterator as any)[Symbol.iterator] = function* () { yield 42; };
  ...
  bench.add('6. isOperation (legacy with iterator fallback)', () => {
    ...
    if (typeof operation !== 'function') return false;
    if (OperationBrand in operation && (operation as any)[OperationBrand] === true) {
      return true;
    }
    return (
      Symbol.iterator in operation &&
  ...
  const currentTask = tasks.find(t => t.name.includes('current implementation'));
  ...
  if (currentTask?.result && legacyTask?.result) {
    ...
    console.log(`Current isOperation is ${speedup.toFixed(2)}x faster than legacy implementation`);
  }

  // Compare brand check vs iterator check
  const brandTask = tasks.find(t => t.name.includes('value[OperationBrand] === true'));
  const iteratorTask = tasks.find(t => t.name.includes('Symbol.iterator in value'));
  
  if (brandTask?.result && iteratorTask?.result) {
    const ratio = iteratorTask.result.mean / brandTask.result.mean;
    console.log(`Brand check is ${ratio.toFixed(2)}x faster than iterator existence check`);
  }

  // Proxy overhead
  const directTask = tasks.find(t => t.name.includes('Direct function call'));
  const proxyTask = tasks.find(t => t.name.includes('Proxy get trap invocation'));
  
  if (directTask?.result && proxyTask?.result) {
    const overhead = proxyTask.result.mean / directTask.result.mean;
    console.log(`Proxy invocation is ${overhead.toFixed(2)}x slower than direct function call`);
  }

  // Generator overhead
  const createTask = tasks.find(t => t.name.includes('Create generator function'));
  const executeTask = tasks.find(t => t.name.includes('Execute generator (next)'));
  
  if (createTask?.result && executeTask?.result) {
    console.log(`Generator creation: ${(createTask.result.mean * 1000).toFixed(3)}ms`);
    console.log(`Generator next(): ${(executeTask.result.mean * 1000).toFixed(3)}ms`);
  }
